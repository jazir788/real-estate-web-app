...
export default function Listing() {
  ...
  return (
    <main>
      ...
      {shareLinkCopy && (
        <p className='p-2 fixed top-[23%] right-[5%] font-semibold border-2 border-gray-400 rounded-md bg-white z-10'>Link Copied</p>
      )}
      <div className="m-4 flex flex-col md:flex-row max-w-6xl lg:mx-auto p-4 rounded-lg shadow-lg bg=white lg:space-x-5  ">
        <div className='w-full h-[200px] lg:h-[400px]'>
          <p className='text-2xl font-bold mb-3 text-blue-900 '> 
            {listing.name} - ${listing.offer ? listing.discountPrice
                            .toString()
                            .replace(/\B(?=(\d{3})+(?!\d))/g, ",") 
          ...
          <div className='flex justify-start items-center space-x-4 w-[75%]'>
            ...
            <p className='bg-green-800 w-full max-w-[200px] rounded-md p-1 text-white text-center font-semibold shadow-md'> {listing.offer &&(
              <p>$ {listing.regularPrice - listing.discountPrice} discount</p>
            )}</p>
          </div>
          <p className='mt-3 mb-6'>  <span className='font-semibold mt-3 mb-3 '> Description - </span> {listing.description}</p>
